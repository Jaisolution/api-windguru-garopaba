...
TABUA_MARES_URL = "https://tabuademares.com/br/santa-catarina/garopaba/previsao/mares"
...
# =====================================================
# BUSCAR MARES - TABUA DE MARES / GAROPABA
# SOMENTE OS PROXIMOS 4 DIAS
# =====================================================

def buscar_mares_tabuademares():
    ...
    try:
        ...
        # Converte o HTML em texto simples sem depender de BeautifulSoup.
        texto = re.sub(
        ...
        texto = re.sub(r"(?i)<br\s*/?>", " ", texto)
        texto = re.sub(r"(?i)</?(?:tr|td|th|div|p|li|h[1-6])[^>]*>", " ", texto)
        ...
        # A pagina mostra blocos como:
        # 24 SET ... 8:09 0,0 m 81 ... 12:34 0,8 m 87 ...
        # Procuramos o inicio de cada dia e limitamos aos 4 primeiros.
        meses = (
            "JAN|FEV|MAR|ABR|MAI|JUN|JUL|AGO|"
            "SET|OUT|NOV|DEZ"
        )

        encontrados_dia = list(
            re.finditer(
                rf"\b([0-3]?\d)\s+({meses})\b",
                texto,
                re.IGNORECASE
            )
        )
        ...
        for pos, encontrado in enumerate(encontrados_dia):

            if len(dias_mares) >= MAX_DIAS:
                break

            inicio = encontrado.start()

            if pos + 1 < len(encontrados_dia):
                fim = encontrados_dia[pos + 1].start()
            else:
                fim = len(texto)

            bloco = texto[inicio:fim]

            dia_num = int(encontrado.group(1))
            mes_nome = encontrado.group(2).upper()

            # Hora + altura + coeficiente.
            eventos_brutos = re.findall(
                r"\b([0-2]?\d:[0-5]\d)\s+"
                r"(-?\d+(?:[.,]\d+)?)\s*m\s+"
                r"(\d{1,3})\b",
                bloco,
            ...
            # Fallback caso o layout deixe de mostrar o coeficiente
            # junto da altura.
            if not eventos_brutos:
                ...
                eventos_brutos = [
                    (hora, altura, None)
                    for hora, altura in simples
                ]

            eventos = []

            for hora, altura_txt, coef in eventos_brutos:

                try:
                    altura = round(
                        float(altura_txt.replace(",", ".")),
                        2
                    )
                except Exception:
                    continue
            ...
            # Evita aceitar um bloco que nao tenha dados reais de mare.
            if not eventos:
                continue
        ...
        # Define ALTA / BAIXA usando a sequencia cronologica completa.
        # Assim funciona mesmo quando um dia comeca pela baixa-mar.
        todos_eventos = []

        for d in dias_mares:
            for evento in d["mares"]:
        ...
        for i, evento in enumerate(todos_eventos):
            ...
            if anterior is None and proximo is not None:
                tipo = "alta" if atual > proximo else "baixa"

            elif proximo is None and anterior is not None:
                tipo = "alta" if atual > anterior else "baixa"

            elif anterior is not None and proximo is not None:
                if atual >= anterior and atual >= proximo:
                    tipo = "alta"
                elif atual <= anterior and atual <= proximo:
                    tipo = "baixa"
                else:
                    # Fallback para manter a alternancia natural.
                    tipo_anterior = todos_eventos[i - 1].get("tipo")
                    tipo = "baixa" if tipo_anterior == "alta" else "alta"
...
def adicionar_mares_aos_dias(dias, dias_mares):

    # Indexa a mare pelo numero do dia.
    indice_mares = {
        str(item.get("dia")): item
        for item in dias_mares
    }

    for dia in dias:

        chave = str(dia.get("data", "")).lstrip("0")

        dados_mare = indice_mares.get(chave)

        if dados_mare:

            dia["mares"] = dados_mare.get(
                "mares",
                []
            )

            dia["mes_mare"] = dados_mare.get(
                "mes"
            )
...
def garopaba():
    ...
    try:
        ...
        # Tabua de Mares - Garopaba / proximos 4 dias
